    ...
    <div className="user">
      ...
      { name.first && name.last
        ...
        { street.number && street.name
          ? <span className="user__col"><strong>Adres:</strong> <div className="user__address"><span>{street.number} {street.name},</span><span>{city}, {country}</span></div></span>
          : <span className="user-item__text user-item__text--error">Użytkownik nie podał adresu.</span>
        }
      </div>
      <div className="user__details">
        <span className="user__col"><strong>Adres e-mail:</strong> <a href={`mailto:${email}`}>{email}</a></span>
        <span className="user__col"><strong>Nr tel.:</strong> <a href={`tel:${cell}`}>{cell}</a></span>
        <span className="user__col"><strong>Data rejestracji:</strong> {date}</span>
      </div>
